...
class TaskbarPanel:
    def __init__(
        ...
        # Legacy P2P fallback until it adopts get_connection_snapshot().
    ...
    def run(self):
        ...
        while not self.loop_terminate:
            ...
            if choice in self.numbered_menu:
                function = self.numbered_menu[choice][1]
    ...
    def get_max_width(self):
        ...
        menu_texts = [
            menu_item[0] if isinstance(menu_item, tuple) else ""
    ...
    def display_menu(self):
        ...
        for menu_item in self.menu_items:
            if isinstance(menu_item, tuple):
                width = max_width
                if (
                    "disconnect" in menu_item[0].lower()
                    or "logs" in menu_item[0].lower()
                ):
                    if "disconnecting" in menu_item[0].lower():
                        width -= 1
                    else:
                        width += 1
                else:
                    width -= 1
                print(f"║ {next(counter):<2}. {menu_item[0]:<{width}} ║")
            else:
                print(f"║ {'─' * (max_width + 4)} ║")
    ...
    def create_menu(self, item_: tuple = None):
        ...
        if (
            ...
            if connection_view.action in (
                ...
                callback = None
            menu_items.insert(0, (connection_view.label, callback))
        ...
        menu_counter = count(1)
        numbered_menu = {
            next(menu_counter): menu_item
    ...
    def update_menu(self, item_: tuple = None, no_dialog=False):
        ...
        if not no_dialog:
            CustomDialog(
                "Menu Updated! Press Enter to Refresh. ⏎",
                msg_type="info",
            ).mainloop()
    ...
    def _update_stats_thread(self):
        while True:
            try:
                ...
                logging.error(f"Failed to update CLI connection status: {error}")
            time.sleep(1)
    ...
    def _wait_to_disconnect(self):
        """Legacy P2P wait retained until P2P adopts the state controller."""
        ...
        try:
            if self.config.data["default_file_download_location"] != "":
                ...
                target_directory = input(
                    "Enter the location to save file(s): "
                ).strip()
            ...
            CustomDialog(
                f"Saving files to: {target_directory}",
                msg_type="info",
            ).mainloop()
