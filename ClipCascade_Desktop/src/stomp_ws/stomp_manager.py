...
class STOMPManager(WSInterface):
    ...
    def set_tray_ref(self, sys_tray: TaskbarPanel):
        """Set the tray reference used by clipboard file-download actions."""
        self.sys_tray = sys_tray
        self.clipboard_manager.set_tray_ref(sys_tray)

    def get_total_timeout(self):
        """Return the current reconnect wait plus handshake timeout in milliseconds."""
    ...
    def get_stats(self) -> str:
        """Return a compact snapshot-derived status for existing tray/CLI surfaces."""
        snapshot = self.connection_controller.snapshot()
    ...
    def connect(self) -> tuple[bool, str]:
        ...
        The controller invokes ``_connect_attempt`` synchronously for explicit
        connects. Automatic retries use the same method from a cancellable timer
        thread, eliminating callback sleeps and recursive reconnect calls.
        """
    ...
    def _connect_attempt(self) -> None:
        ...
        try:
            ...
            if error.code in (
                ...
                # Startup/login code expects a synchronous failure and owns the
                # next explicit retry. Do not launch a hidden background loop.
