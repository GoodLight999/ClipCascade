...
class ConnectionController:
    ...
    def _enter_reconnect_wait_locked(
        ...
        if stale_handle is not None:
            # A valid transition should not normally replace an existing timer.
            # Cancelling while holding the state lock is avoided, so cancellation
            # is performed immediately after releasing it through a diagnostic
            # path in integrations. The stale generation already makes it inert.
            self._diagnose(
    ...
    def _install_retry(self, generation: int, delay_seconds: float) -> None:
        ...
        except Exception as exception:  # scheduler failure is a local fatal error
    ...
    def _invoke_stop_action(self) -> None:
        try:
            ...
            # Do not leave the application permanently stuck in STOPPING.
            self.stop_completed()
    ...
    def _cancel_handle(handle: ScheduledHandle) -> None:
        try:
            ...
            # Cancellation is best-effort; generation checks keep stale callbacks inert.
            pass
    ...
    def _diagnose(self, code: str, message: str) -> None:
        try:
            ...
            # Diagnostics must never be able to corrupt connection state.
            pass
