...
- Confirmed that screen-off/locked SMS and email verification-code delivery is already an explicit product goal, verification requirement, Definition-of-Done item, handoff requirement, and test-matrix section.
...
1. `852141452014ea7265ebbf626cf004c8b472e84b` is not used here; the actual redundant boot recovery commit is `852141452014ea7265ebbf7587f4d56e71f525b8`. It adds a delayed WorkManager heartbeat while preserving immediate Headless JS recovery. Android run `28304486580`: success.
...
3. `eb8dfe5a3de9d8e00faffbea5fb541b34dd1af5b`, `1b95ccf7074d614a97521d519f3bb9b842a1f0f6`, and `230a10562456dc96d0f354e2d78e01123d10a800` stop treating `Disconnected` as a valid connected state in recovery, clipboard dispatch, and verification dispatch. Android runs `28304618791`, `28304684578`, and `28304755556`: success.
...
- A separate display-only status localizer translates connection/login/P2P status for Japanese users without changing internal protocol/storage tokens.
...
- Japanese: authentication/confirmation numbers, login/sign-in, one-time passwords, identity verification, two-step authentication, security codes, full-width normalization.
- English: OTP, one-time password/passcode, verification/security/auth/login/sign-in/confirmation/access/two-factor language, code-before-keyword and code-after-keyword forms.
...
- Notification text containing a transaction amount or destination email address can still yield a separate legitimate OTP.

Trial-and-error record:

1. Commit `000d964408d7f4824cd0f16fb9d47954ccddf425` failed Android run `28307536157`. The grouped-candidate pattern joined ordinary words with following values, producing false candidates such as `ON202606` from `on 2026-06-27`, and swallowed a valid alphanumeric code with its preceding word.
2. Commit `acd5cb9cf0dad674fb58b1d1b691d4f3cdf941ee` restricted space grouping to numeric groups and hyphen grouping to alphanumeric groups.
3. Commit `aa799b3553b2bfa160f793324972b072edead18b` expanded the Japanese/English positive and negative corpus; Android run `28307672613`: success.
4. Commit `9d06c1b175aaff118f09be7df929486155cc67e9` added transaction-notification positive cases; Android run `28307814762`: success.
5. Commit `abb9ca720ab728c56d8ee490132f0c9c1f6ae572` failed Android run `28307830787` only in the new bilingual-bundle assertion. Metro escaped Japanese literals, so raw Japanese grep against the bundle was invalid; compilation was never reached in that run.
6. Commit `9b4e5da9b62c6a3054421c90697cf6363ce67134` moved bilingual/no-ADB assertions to transformed `App.js`/`AppRoot.js`. In run `28307959797`, status localization, source assertions, bundle creation, unit tests, APK assembly, embedded-bundle verification, and artifact upload all succeeded.
7. Commits `cbae051f9b88a3b2f0584f615aabf3b61f61dee3` and `bc7cbfc7005fa925662bc0dc3e9969e0b799a9a2` further distinguish real OTPs from generic codes and preserve OTPs when an email address is present. Final CI result must be recorded after completion.

Remaining mandatory work:

- Fetch matching Android and Windows artifacts from the final documentation HEAD.
- Install the new APK on HONOR 400 Pro.
- Follow the five-step setup from a fresh/permission-reset state.
- Run the synthetic notification test in Extended P2P and confirm the settings status reaches `acknowledged` only after the Windows clipboard contains the synthetic value.
