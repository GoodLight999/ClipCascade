...
Current phase: `.21-alpha.1` failed both decisive HONOR tests. `.22-alpha.1 / 320127` has passed pre-final Android validation on Draft PR #2 and must now receive exact final-branch Android and Windows CI before target testing.
...
`.21-alpha.1` is therefore a failed target build despite Android and Windows CI success.
...
4. Accessibility accepts broad candidate events, including selection change and generic clicks.
...
- new `ClipboardAcquisitionService` native foreground service;
...
Temporary Draft PR #2 exists only for validation and must never be merged.

- staging SHA: `f8ec5245f9bd2eeaac6400a4f0f57d85ad6d429e`;
- Android CI `29915789910`: success;
- production transforms: success;
- source invariants: success;
- JS bundle: success;
- Kotlin unit tests: success;
- main APK: success;
- external test-sender APK: success;
- both stable-signature checks: success.
...
- helper SHA-256 `61e9183d4eb93fedb80e1ea0b624663516f61fc2a2e5752df985aee9066b6e2b`;
- helper size `831357` bytes.

These are staging artifacts only. Final exact-SHA artifacts must be recorded separately.
...
- Draft PR #2 Android `29915789910`: `.22` pre-final validation green.
- no force push, Ready conversion, merge, or auto-merge occurred.

## Next actions

1. Fast-forward `stability-mobile-otp` without force only after final document/release definitions are ready.
2. Trigger exact final-branch Android and Windows CI.
3. Record final artifact IDs, hashes, sizes, signatures, and expiry.
4. Close Draft PR #2 without merge.
5. Keep PR #1 Open and Draft.
6. Install both `.22` APKs and run component, external-listener, and background-copy tests in that order.
