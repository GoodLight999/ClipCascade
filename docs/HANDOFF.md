...
Highest priority: reliable Android background clipboard delivery through the existing React Native/Notifee foreground-service/STOMP transport.
...
- readable setup and payload-free diagnostics;
...
- Existing foreground service remains sole transport owner.
...
### Shizuku fork recovery

Previous APK:

- waited only for passive Binder delivery;
- hardcoded official manager open/download behavior;
...
- Binder, permission, UserService, and UID absent;
...
- reports detected manager label/version/package without payload data;
...
- Existing UI displays queue state and retry countdown.
...
- Payloads, hashes, server URLs, accounts, credentials, cookies, and keys are excluded.
...
Packaged-runtime inspection confirmed Binder-request/reprobe markers, compatible-manager diagnostics, dedicated theme/colors, recovery-project links, and absence of official Shizuku download and Sathvik-Rao product URLs.
...
Documentation-only operations accidentally shortened or replaced this Handoff in commits:
...
Product code, CI, and artifacts were unaffected. This commit restores the canonical Handoff. Do not use the temporary commits as continuation sources. PR body synchronization was intentionally stopped after these mistakes.

## Still unproven

Android:

- Binder delivery from the user's exact fork under current stealth settings;
